...
def read_input(line, street_status):
    ...
    print('Error: The command format is not correct.', file=sys.stderr)
...
def print_output(num_vex_list, edge_list):
    print("V = {")
    for i, vertex in enumerate(num_vex_list, start = 1):
        print("  {}: {}".format(i, vertex))
    print("}")

    print("E = {")
    if len(edge_list)>0:
        for edge_line_seg in edge_list:
            ...
            print("  <{0},{1}>,".format(index1+1, index2+1))
...
class Streets:
    ...
    def add(self, street_name, coord_num_list):
        if street_name in self.street_dict:
            print("Error: 'a' specified for a street that already exist.", file=sys.stderr)
